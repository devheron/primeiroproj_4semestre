...
public class BibliotecaController {
	...
	@Autowired // esse comando faz com que abra uma nova instancia sozinho
	// Annotation
	...
	public ResponseEntity<String> save(@RequestBody Biblioteca biblioteca){
		try {
			...
			return new ResponseEntity<>("Tem erro aqui!"+e.getMessage(), HttpStatus.BAD_REQUEST );
		}
	}
	...
	public ResponseEntity<List<Biblioteca>> findAll(){
		try {
			List<Biblioteca> listaBiblioteca = this.bibliotecaService.findAll();
			return new ResponseEntity<>(listaBiblioteca, HttpStatus.OK);
		} catch (Exception e) {
			return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST );
		}
	}
